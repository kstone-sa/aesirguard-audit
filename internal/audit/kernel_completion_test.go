...
func TestKernelSecurityRecordsRequireCompoundEventBoundary(t *testing.T) {
	for _, kind := range []string{"CONFIG_CHANGE", "FEATURE_CHANGE", "MAC_STATUS", "MAC_POLICY_LOAD", "AVC", "SECCOMP", "BPF", "CAPSET", "BPRM_FCAPS"} {
		for _, terminal := range []string{"EOE", "PROCTITLE"} {
			for _, reverse := range []bool{false, true} {
				t.Run(kind+"/"+terminal+map[bool]string{true: "/reversed", false: "/forward"}[reverse], func(t *testing.T) {
					...
					// audit_log_exit() emits auxiliary security evidence and PATH
					// before PROCTITLE; only EOE follows it in the kernel grammar.
					line := "type=" + terminal + ` msg=audit(100.0:1):`
					if terminal == "PROCTITLE" {
						line += ` proctitle="tool"`
					}
					events := a.Add(mustParseRecord(t, line))
