...
func TestWithHumanMessageRendersCISAuditFamilies(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "sudo scope",
			event:   CanonicalEvent{Event: CanonicalEventMeta{Action: "change_privilege_scope"}, Actor: &CanonicalActor{User: "mario"}, Paths: []CanonicalPath{{Name: "/etc/sudoers"}}},
			message: "mario changed sudo privilege scope at /etc/sudoers",
		},
		{
			name:    "sudo log",
			event:   CanonicalEvent{Event: CanonicalEventMeta{Action: "write_sudo_log"}, Actor: &CanonicalActor{User: "mario"}, Paths: []CanonicalPath{{Name: "/var/log/sudo.log"}}},
			message: "mario modified the sudo log at /var/log/sudo.log",
		},
		{
			name:    "time",
		...
		{
			name:    "failed access",
			event:   CanonicalEvent{Event: CanonicalEventMeta{Action: "access", Success: &failure}, Actor: &CanonicalActor{User: "mario"}, Paths: []CanonicalPath{{Name: "/etc/shadow"}}, Process: &CanonicalProcess{Syscall: "openat"}},
			message: "mario failed to access a file at /etc/shadow using openat",
		},
		{
			name:    "identity",
		...
		{
			name:    "permissions",
			event:   CanonicalEvent{Event: CanonicalEventMeta{Action: "change_permissions"}, Actor: &CanonicalActor{User: "mario"}, Paths: []CanonicalPath{{Name: "/srv/data"}}, Process: &CanonicalProcess{Syscall: "chmod"}},
			message: "mario changed file permissions or ownership on /srv/data using chmod",
		},
		{
			name:    "mount",
		...
		{
			name:    "login",
			event:   CanonicalEvent{Event: CanonicalEventMeta{Action: "update_login_record"}, Paths: []CanonicalPath{{Name: "/run/faillock/mario"}}},
			message: "A process updated login records at /run/faillock/mario",
		},
		{
			name:    "delete",
			event:   CanonicalEvent{Event: CanonicalEventMeta{Action: "delete"}, Actor: &CanonicalActor{User: "mario"}, Paths: []CanonicalPath{{Name: "/tmp/evidence"}}},
			message: "mario deleted a file at /tmp/evidence",
		},
		{
			name:    "rename",
			event:   CanonicalEvent{Event: CanonicalEventMeta{Action: "rename"}, Actor: &CanonicalActor{User: "mario"}, Paths: []CanonicalPath{{Name: "/tmp/old", NameType: "DELETE"}, {Name: "/tmp/new", NameType: "CREATE"}}},
			message: "mario renamed a file from /tmp/old to /tmp/new",
		},
		{
			name:    "MAC policy",
...
func TestWithHumanMessageDoesNotInventUnknownOutcome(t *testing.T) {
	...
	got := WithHumanMessage(event)
	if got.Message != "mario attempted to change system time using clock_settime" {
...
func TestWithHumanMessageDescribesFailedExecveAccurately(t *testing.T) {
	...
	got := WithHumanMessage(event)
	if got.Message != "mario failed to execute /missing/tool" {
		t.Fatalf("message = %q", got.Message)
	}
}
