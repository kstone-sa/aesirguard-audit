...
| Kernel Audit attribution: `LOGIN` | `session` / `set_audit_attribution`; `process.audit_attribution.old/new` retains loginuid, enriched user, session ID and explicit unset flags; own numeric result (0/1) | Sets or attempts to set kernel loginuid/session attribution; never equivalent to `USER_LOGIN`. Remains correlated with SYSCALL/PROCTITLE until EOE, trailing PROCTITLE, event-time watermark or an incomplete flush; new values are applied only on success |
